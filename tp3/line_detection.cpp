...
void line_detection::thread(void) {
	...
	//Communication filter thread (Protocole handshaking)
	int ready = 0;
	int ack = 0;
	...
	while(1) {
		...
		RegisterWrite(REGISTERFILE0_ID, 17, &height_scaled);
		RegisterWrite(REGISTERFILE0_ID, 18, &width_scaled);
		RegisterWrite(REGISTERFILE0_ID, 19, &m_pixels_buffer_out_resize);
		RegisterWrite(REGISTERFILE0_ID, 20, &m_pixels_buffer_out_filter);

		//Appel Thread filter
		ready = 1;
		RegisterWrite(REGISTERFILE0_ID, 21, &ready);

		// - WIP - Travail du thread filter

		//Communication Thread filter : Attente début travail
		do {
			RegisterRead(REGISTERFILE0_ID, 22, &ack);
		} while (ack == 0);

		//Communication Thread filter
		ready = 0;
		RegisterWrite(REGISTERFILE0_ID, 21, &ready);


		//Communication Thread filter : Attente travail fini
		do {
			RegisterRead(REGISTERFILE0_ID, 22, &ack);
		} while (ack == 1);
