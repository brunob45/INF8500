...
# endif
    ...
    std::string parseString(const std::string& val, int& radix) {
        radix = 10;
        int len = val.length();
        if (len < 2)
            return val;
        ...
        if (radix == 10) {
            ...
            if (expPos < 0) {
                ans += val.substr(startPos, len-startPos);
                return ans;
            } else {
                ans += val.substr(startPos, expPos-startPos);
                ap_private<64, true> apExp(val.substr(expPos+1, len-expPos-1), 10);
                exp = apExp.to_int();
            }
        } else {
            int expPos = val.find('p');
            if (expPos < 0) expPos = val.find('P');
            if (expPos < 0) {
        ...
        if (decPos + exp >= (int)ans.length()) {
            ...
            for (; i<decPos + exp; ++i)
                ans += '0';
            return ans;
        } else if (decPos + exp < isNegative) {
            std::string dupAns = "0.";
            if (ans[0] == '-')
                dupAns = "-0.";
            for (int i=0; i<isNegative-decPos-exp; ++i)
                dupAns += '0';
            for (int i=isNegative; i<ans.length(); ++i)
                if (ans[i] != '.')
                    dupAns += ans[i];
            return dupAns;
        }
    ...
    void fromString(const std::string& val) {
        int radix;
        std::string s = parseString(val, radix);
        fromString(s, radix);
    }

    void fromString(const std::string& val, unsigned char rd) {
        assert(rd == 2 || rd == 8 || rd == 10 || rd == 16);
        V = 0;
        int startPos = 0;
        ...
        char *ctmp = (char*)malloc((endPos-startPos+1)*sizeof(char));
        int i=0;
        for (int j=startPos; j!=endPos; ++j)
            if (val[j] != '.')
                ctmp[i++] = val[j];
        ctmp[i] = '\0';
        std::string validNumString(ctmp);
        enum {MAX_HALF_LEN = 1+((_AP_W>_AP_I)?(_AP_I>0?_AP_W:(_AP_W-_AP_I)):(_AP_I))};
        ap_private<MAX_HALF_LEN, false> tmpApPrivate(validNumString, rd);
        ap_fixed_base<MAX_HALF_LEN*2, MAX_HALF_LEN, false> tmpApFixed = tmpApPrivate;

        int exp = decPos - endPos;
        //if (exp > 0) --exp;
        if (exp < 0) ++exp;
        if (exp > 0)
            for (int i=0; i<exp; ++i)
                tmpApFixed = tmpApFixed * rd;
        else if (exp < 0)
            for (int i=0; i<-exp; ++i)
                tmpApFixed = tmpApFixed / rd;
        ...
        if((ireg&0x7fffffffffffffffULL)!=0) {          
            ...
            if (F2==_AP_F )
                ...
                if(sh_amt >= DOUBLE_MAN+2)
                    ...
                    V=(man>>sh_amt) | ((man & 1ULL<<(DOUBLE_MAN+1)).to_bool() ? (DOUBLE_MAN_MASK>>(DOUBLE_MAN+2-sh_amt) <<(DOUBLE_MAN+2-sh_amt)):0);
            ...
            if((_AP_O != AP_WRAP || _AP_N != 0) && 
                ...
                bool deleted_zeros = _AP_S2?true:!carry, 
                     deleted_ones = true;
                bool neg_src;
                const bool lD=(_AP_I2>_AP_I) && (_AP_W2-_AP_I2+_AP_I>=0) && (man & (1ULL <<(DOUBLE_MAN+2-_AP_I2+_AP_I))).to_bool();
                ...
                if (pos1 < _AP_W2) {
                    ...
                    if (pos1>=0) {
                        ap_private<DOUBLE_MAN+2,_AP_S> Range1=
                            ap_private<DOUBLE_MAN+2,_AP_S>((man >> pos1) | ((1ULL<<(DOUBLE_MAN+1)&man).to_bool() ? (DOUBLE_MAN_MASK >> (DOUBLE_MAN+2-pos1) <<(DOUBLE_MAN+2-pos1)):0));
                    ...
                    if (pos2<_AP_W2 && pos2>=0) {
                        ap_private<DOUBLE_MAN+2, _AP_S> Range2=
                            ap_private<DOUBLE_MAN+2, _AP_S>((man >> pos2) | ((1ULL<<(DOUBLE_MAN+1)&man).to_bool() ? (DOUBLE_MAN_MASK >> (DOUBLE_MAN+2-pos2) <<(DOUBLE_MAN+2-pos2)):0));
    ...
    if (_AP_W <= _AP_I) 
        ...
        if (radix == 10) {
            ...
            if (_AP_I > 0) {
                ...
                int_part = tmp.to_ap_private();
                str += int_part.to_string(10, false);
            } else
                str += '0';
...
INLINE std::string scientificFormat(std::string& input) {
    ...
    int lastNonZeroPos = input.length() - 1;
    ...
    if (firstNonZeroPos != lastNonZeroPos) {
        ans += '.';
...
INLINE std::string reduceToPrecision(std::string& input, int precision) {
    ...
    for (int i=0; i<inputLen && isZero; i++)
    ...
    if (decPosition < 0) //When this is integer
        truncBitPosition = dupInput.length();
    else 
        for (truncBitPosition = dupInput.length()-1;
             truncBitPosition >=0;
             --truncBitPosition) {
