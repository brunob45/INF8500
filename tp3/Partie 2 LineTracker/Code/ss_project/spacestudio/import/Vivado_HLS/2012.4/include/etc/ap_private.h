...
INLINE bool InvalidDigit(const char* str, unsigned len, unsigned start, unsigned radix) {
    unsigned i;
    for (i = start; i < len; ++i) 
...
INLINE void ap_parse_sign(const char* str, uint32_t &base, bool &neg) {
...
INLINE void ap_parse_prefix(const char* str, uint32_t &offset, uint32_t &radix) {
    if (str[0] == '0') {
        switch (str[1]) {
            case 'b':
...
public:
    ...
    ap_private(const char* val) {
        ...
        bool neg = false;        
        uint32_t radix = 10;
        ap_private_ops::ap_parse_sign(strp, base, neg);
        ap_private_ops::ap_parse_prefix(strp + base, offset, radix);  

        if ((radix != 10 && neg) ||
                (strLen - base - offset <= 0) ||
                ap_private_ops::InvalidDigit(strp, strLen, base + offset, radix))  {
    ...
    ap_private(const std::string& val, uint8_t radix=2, int base=0, int offset=0) {
    ...
    ap_private(const char strStart[], uint32_t slen, uint8_t radix, int base=0, int offset=0) {
    ...
    ap_private(const volatile ap_private<_AP_W1, _AP_S1, _AP_N1>& that) {
        operator = (const_cast<const ap_private<_AP_W1, _AP_S1, _AP_N1>& >(that));
    } 

    template<int _AP_W1, bool _AP_S1, int _AP_N1>
    ap_private(const ap_private<_AP_W1, _AP_S1, _AP_N1>& that) {
        operator = (that);
    } 

    template<int _AP_W1, bool _AP_S1>
    explicit ap_private(const ap_private<_AP_W1, _AP_S1, 1>& that) {
...
#undef isdigit
    ...
    INLINE RetValType getVal() const{
        return *pVal;
    }
...
#undef OP_BIN_LOGIC_ASSIGN_AP
    ...
    ap_private& operator*=(const ap_private<_AP_W1, _AP_S1, _AP_N1>& RHS) {
        ...
        ap_private_ops::mul(dest, pVal, lhsWords, dupRHS.pVal, rhsWords, destWords);
...
#undef OP_BIN_LOGIC_AP
    ...
    INLINE typename RType<_AP_W1,_AP_S1>::plus operator+(const ap_private<_AP_W1, _AP_S1, _AP_N1>& RHS) const {
        ...
        for (int i=0; i<_AP_N1; ++i)
            RHSpVal[i] = RHS.get_pVal(i);
        bool carry = ap_private_ops::add(Result.pVal, pVal, RHSpVal, (RType<_AP_W1,_AP_S1>::plus_w + 63) / 64, _AP_N, _AP_N1, _AP_S, _AP_S1);
    ...
    INLINE typename RType<_AP_W1,_AP_S1>::minus operator-(const ap_private<_AP_W1, _AP_S1, _AP_N1>& RHS) const {
        ...
        for (int i=0; i<_AP_N1; ++i)
            RHSpVal[i] = RHS.get_pVal(i);
        bool borrow = ap_private_ops::sub(Result.pVal, pVal, RHSpVal, (RType<_AP_W1,_AP_S1>::minus_w + 63) / 64,  _AP_N, _AP_N1, _AP_S, _AP_S1);
    ...
    typename RType<_AP_W1, _AP_S1>::mult operator*(const ap_private<_AP_W1, _AP_S1, _AP_N1>& RHS) const {
        ...
        // Perform the long multiply
        ap_private_ops::mul(dest, dupLHS.pVal, lhsWords, dupRHS.pVal, rhsWords, destWords);
...
#undef OP_ASSIGN_AP
    ...
    ap_private sqrt() const {
        ...
        if (magnitude <= 5) {
            ...
            return ap_private<_AP_W, _AP_S, _AP_N>(/*BitWidth,*/ results[get_VAL()]);
        }
    ...
    ap_private Add(const ap_private& RHS) const {
        ap_private Result(0);
        ap_private_ops::add(Result.pVal, pVal, RHS.pVal, _AP_N, _AP_N, _AP_N, _AP_S, _AP_S);
        Result.clearUnusedBits();
        return Result;
    }
    ...
    ap_private Sub(const ap_private& RHS) const {
        ap_private Result(0);
        ap_private_ops::sub(Result.pVal, pVal, RHS.pVal, _AP_N, _AP_N, _AP_N, _AP_S, _AP_S);
        Result.clearUnusedBits();
        return Result;
    }
...
std::string ap_private<_AP_W, _AP_S, _AP_N>::toString(uint8_t radix, bool wantSigned) const {
    ...
    if (radix != 10) {
        ...
        if (*this == (uint64_t)(0))
            ...
            while (tmp.ne(zero)) {
                unsigned digit = tmp.get_VAL() & mask;
...
public:
  ...
  typedef typename valtype<(_AP_W+7)/8, _AP_S>::Type ValType;
    ...
    INLINE uint64_t get_pVal(int i) const {
        return VAL;
    }
    INLINE void set_pVal(int i, uint64_t value) {
        VAL = (ValType)value;
...
#define CTOR(TYPE)                                                \
    INLINE  ap_private(TYPE v) : VAL((ValType)(uint64_t)v) {        \
        clearUnusedBits(); \
    }
    CTOR(int)
    CTOR(bool)
...
#undef CTOR
    ...
    ap_private(const ap_private& that) : VAL(that.VAL) {
        clearUnusedBits();
    }

    template<int _AP_W1, bool _AP_S1>
    ap_private(const ap_private<_AP_W1, _AP_S1, 1>& that) : VAL((ValType)(uint64_t)that.VAL) {
        clearUnusedBits();
    }

    template<int _AP_W1, int _AP_N1, bool _AP_S1>
    ap_private(const ap_private<_AP_W1, _AP_S1, _AP_N1>& that) : VAL((ValType)that.pVal[0]) {
        clearUnusedBits();
    }

    template<int _AP_W1, bool _AP_S1>
    ap_private(const volatile ap_private<_AP_W1, _AP_S1, 1>& that) : VAL((ValType)(uint64_t)that.VAL) {
        clearUnusedBits();
    }

    template<int _AP_W1, int _AP_N1, bool _AP_S1>
    ap_private(const volatile ap_private<_AP_W1, _AP_S1, _AP_N1>& that) : VAL((ValType)that.pVal[0]) {
        clearUnusedBits();
    }
    ...
    INLINE bool operator==(const ap_private<_AP_W1, _AP_S1, 1>& RHS) const { 
        return ((uint64_t)VAL == (uint64_t)RHS.VAL); 
    }

    INLINE  bool operator==(const ap_private<_AP_W, _AP_S>& RHS) const { return VAL == RHS.VAL; }
    INLINE  bool operator==(const ap_private<_AP_W, !_AP_S>& RHS) const { return (uint64_t)VAL == (uint64_t)RHS.VAL; }
    ...
    ap_private<_AP_W, _AP_S, 1> getLoBits(uint32_t numBits) const {
        ap_private<_AP_W, _AP_S, 1> ret(((uint64_t)VAL) << (BitWidth - numBits));
        ret = (ret)>>(BitWidth - numBits);
        return ret;
    ...
    INLINE  ap_private urem(const ap_private<_AP_W, _AP_S2>& RHS) const {
        ...
        return ap_private(((uint64_t)VAL)%((uint64_t)RHS.VAL));
    }
