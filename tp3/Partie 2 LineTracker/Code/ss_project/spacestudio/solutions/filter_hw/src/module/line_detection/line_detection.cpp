...
//		convolution_filter(height_scaled, width_scaled, KERNEL_MATRIX_SIZE);
		...
		ModuleWrite(FILTER0_ID, SPACE_BLOCKING, &address_2_read);
		ModuleWrite(FILTER0_ID, SPACE_BLOCKING, &address_2_write);
		...
		//Hough
		output_memory_offset = maximum_nb_lines * sizeof(line_points) * m_head_out_buff;
		address_2_write = output_address + output_memory_offset;
